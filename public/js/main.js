document.addEventListener('DOMContentLoaded', function () {
    ...
    function calculateComplexity(code) {
        ...
        const patterns = {
            ...
            methods: /\b(void|int|double|float|char|bool|string|auto)\s+\w+\s*\([^)]*\)/g,
            recursion: /\w+\s*\([^)]*\)\s*{[^}]*\1\s*\(/g,
            templates: /template\s*<[^>]*>/g,
            operators: /operator\s*[^(]+\s*\(/g
        };
        ...
        complexity += (code.match(patterns.templates) || []).length * 2;
        complexity += (code.match(patterns.operators) || []).length * 1.5;
    ...
    document.getElementById('downloadBtn').addEventListener('click', async () => {
        ...
        doc.text('Analysis:', 10, yPosForAnalysis);
